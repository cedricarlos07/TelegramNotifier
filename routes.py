...
from flask import render_template, request, redirect, url_for, jsonify, flash, Blueprint
...
• Et bien plus encore!"""
    ...
    def students():
        ...
        total_groups = len(telegram_groups)
        total_courses = len(courses)
    ...
    @app.route('/students/extract-from-telegram', methods=['POST'])
    @login_required
    def extract_students_from_telegram():
        """Extrait les étudiants depuis les groupes Telegram"""
        try:
            group_id = request.form.get('group_id')
            if not group_id:
                return jsonify({'success': False, 'message': 'ID du groupe Telegram requis'})
            ...
            if not members:
                return jsonify({'success': False, 'message': 'Aucun membre trouvé dans le groupe'})
            ...
            for member in members:
                ...
                student = Student.query.filter_by(telegram_id=member.id).first()
                
                if student:
                    # Mettre à jour les informations existantes
                    student.first_name = member.first_name
                    student.last_name = member.last_name
                    student.username = member.username
                    student.telegram_id = member.id
                    updated_count += 1
                else:
            ...
            return jsonify({
                'success': True,
                'message': f'{added_count} étudiants ajoutés, {updated_count} mis à jour'
            })
            
        except Exception as e:
            return jsonify({'success': False, 'message': str(e)})

    @app.route('/students/assign-course', methods=['POST'])
    @login_required
    def assign_student_to_course():
        ...
        try:
            ...
            if not student_id or not course_id:
                return jsonify({'success': False, 'message': 'ID étudiant et cours requis'})
            ...
            if course in student.courses:
                return jsonify({'success': False, 'message': 'L\'étudiant est déjà assigné à ce cours'})
            ...
            return jsonify({
                'success': True,
                'message': f'Étudiant {student.first_name} {student.last_name} assigné au cours {course.course_name}'
            })
            
        except Exception as e:
            return jsonify({'success': False, 'message': str(e)})

    @app.route('/students/remove-course', methods=['POST'])
    @login_required
    def remove_student_from_course():
        ...
        try:
            ...
            if course not in student.courses:
                return jsonify({'success': False, 'message': 'L\'étudiant n\'est pas assigné à ce cours'})
            ...
            return jsonify({
                'success': True,
                'message': f'Étudiant {student.first_name} {student.last_name} retiré du cours {course.course_name}'
            })
